...
final class ModelState: ObservableObject, Identifiable {
    ...
    init(modelConfig: ModelConfig,
         ...
         startState: AppState,
         chatState: ChatState,
         chatStorage: ChatStorage? = nil) {
        self.modelConfig = modelConfig
        self.modelLocalBaseURL = modelLocalBaseURL
        self.startState = startState
        self.chatState = chatState
        self.chatStorage = chatStorage ?? ChatStorage()
    }
    ...
    func startChat(chatState: ChatState, chatStorage: ChatStorage? = nil) {
        // Update legacyUseImage based on model capabilities
        if let supportsVision = modelConfig.supportsVision {
            chatState.legacyUseImage = supportsVision
        }
    ...
    func switchToClearing() {
        if modelDownloadState == .paused {
            ...
            if chatState.modelID == modelConfig.modelID {
                chatState.requestTerminateChat(chatStorage: chatStorage) { [weak self] in
                    self?.clear()
                }
            } else {
                clear()
